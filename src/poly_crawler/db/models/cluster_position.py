...
from sqlalchemy.dialects.postgresql import JSONB, UUID as UUIDColumn
...
class ClusterPosition(UUIDMixin, TimestampMixin, Base):
    ...
    cluster_id: Mapped[UUID] = mapped_column(ForeignKey("clusters.id", ondelete="CASCADE"), nullable=False, index=True)
    ...
    market_tags: Mapped[dict] = mapped_column(JSONB, default=list, nullable=False)
    ...
    sibling_balances: Mapped[dict] = mapped_column(JSONB, default=dict, nullable=False)
    tp_sl_suspended: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)
    ...
    config_snapshot_id: Mapped[UUID | None] = mapped_column(
        UUIDColumn(as_uuid=True), ForeignKey("config_snapshots.id"), nullable=True
    )
    ...
    trades = relationship("PaperTrade", back_populates="position", cascade="all, delete-orphan")
