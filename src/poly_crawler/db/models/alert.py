...
class Alert(UUIDMixin, TimestampMixin, Base):
    ...
    parent_id: Mapped[UUID | None] = mapped_column(ForeignKey("parents.id"), nullable=True, index=True)
    account_id: Mapped[UUID | None] = mapped_column(ForeignKey("accounts.id"), nullable=True)
    cluster_id: Mapped[UUID | None] = mapped_column(ForeignKey("clusters.id"), nullable=True, index=True)
    ...
    is_false_positive: Mapped[bool | None] = mapped_column(Boolean, default=None, nullable=True, index=True)
