# PolyCrawler — Full Product & Technical Spec (v0.1.5)

> **Archived reference.** Complete behavioral specification. For day-to-day development, use [docs/phases/_index.md](../phases/_index.md). This file is the canonical archive when you need the full text in one place.
...
| `SIGNAL` | Entry rules passed but human review is required. Waits for approval or rejection. |
...
| `SKIPPED` | Review rejected, or cluster has exposure we chose not to mirror. |

### 6.3 Persisted fields (`cluster_positions` row)
...
> **Polling note:** Exit detection uses configurable poll intervals, not real-time websockets. Missed events between polls are corrected by position reconciliation (§8.2).
